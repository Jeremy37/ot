...
EXP_DIR=$JS/experiment/transcribed/BOB_CLU_CCDC6_wt_spike
#md $EXP_DIR
cd $EXP_DIR
...
# to samples.
META=BOB_CLU_CCDC6_wt_spike.genie.meta.tsv

# Create links to fastq files
mkdir fastq
cd fastq
find $JS/experiment/transcribed/fastq/MiSeq_Walkup_211-139782655 -type f -name '*.fastq.gz' -exec ln -s {} . \;
cd $EXP_DIR
mkdir fastq_flash

# Metadata document here:
# https://docs.google.com/spreadsheets/d/1l_yoXcONL-D5fP8AurD-l0Ly6gItscpHDp4z0bne1RI/edit#gid=1196408431
...
Rscript $JS/src/experiment/getFlashInput.R BOB_CLU_CCDC6_wt_spike.regions.tsv $META 150 > BOB_CLU_CCDC6_wt_spike.flash_input.tsv

sed '1d' BOB_CLU_CCDC6_wt_spike.flash_input.tsv | submitJobs.py --MEM 300 --ncores 1 -q normal -j merge_fastq_flash \
...
BAMDIR=bam_amplicon
sed '1d' BOB_CLU_CCDC6_wt_spike.flash_input.tsv | cut -f 1,9 | submitJobs.py --MEM 200 --ncores 2 -q normal -j align_fastq_flash_amplicon \
