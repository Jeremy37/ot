...
EXP_DIR=$JS/experiment/transcribed/BOB_CLU_CCDC6
#md $EXP_DIR
cd $EXP_DIR
...
# to samples.
META=BOB_CLU_CCDC6.genie.meta.tsv

# Copy fastq files from Miseq folders all into a single folder
ln -s ../fastq/BOB/ fastq
mkdir fastq_flash

# Metadata document here:
# https://docs.google.com/spreadsheets/d/1l_yoXcONL-D5fP8AurD-l0Ly6gItscpHDp4z0bne1RI/edit#gid=54704208
...
Rscript $JS/src/experiment/getFlashInput.R BOB_CLU_CCDC6.regions.tsv $META 150 > BOB_CLU_CCDC6.flash_input.tsv

sed '1d' BOB_CLU_CCDC6.flash_input.tsv | submitJobs.py --MEM 300 --ncores 1 -q normal -j merge_fastq_flash \
...
BAMDIR=bam_amplicon
sed '1d' BOB_CLU_CCDC6.flash_input.tsv | cut -f 1,9 | submitJobs.py --MEM 200 --ncores 2 -q normal -j align_fastq_flash_amplicon \
...
################################################################################
# Do the same but with new data which has WT cells spiked in
