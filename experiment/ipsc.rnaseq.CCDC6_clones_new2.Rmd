...
# Raw featureCounts file prepared by JS
counts.fname = file.path(root, "CCDC6_clones_all.counts.tsv.gz")

# Raw featureCounts file prepared by MJB
counts.fname.mjb = file.path("/Users/jeremys/work/opentargets/ipsc/hipsci_var/RAW/IPSc-Full.featureCounts.genes.counts.unique.tsv_counts.tsv.gz")
# Normalised expression file by MJB
expr.norm.fname.mjb = file.path("/Users/jeremys/work/opentargets/ipsc/hipsci_var/Exp/OT_QC_CS.CovariatesRemoved_Rel.txt.gz")
# PCs for our samples (and HIPSCI samples) determined across whole
# HIPSCI dataset
expr.pcs.mjb = file.path("/Users/jeremys/work/opentargets/ipsc/hipsci_var/PCs/FullFreeze_20180626_All_TPM2.PcScores_Rel.txt.gz")
...
rpkm.mat = getRPKMs(normalized_counts)
rpkm.df = cbind(data.frame(gene_id = rownames(normalized_counts)), rpkm.mat)

ipsc.rpkm.avgs.df = getExprSummary(rpkm.mat[, ipsc_samples])
mg.rpkm.avgs.df = getExprSummary(rpkm.mat[, mg_samples])

# Save RPKMs for individual samples
...
ipsc.tpm.avgs.df = getExprSummary(tpm.mat[, ipsc_samples])
mg.tpm.avgs.df = getExprSummary(tpm.mat[, mg_samples])
#expressedGenes.ipsc = ipsc.tpm.avgs.df %>% filter(mean >= 1) %>% .$gene_id
#expressedGenes.mg = mg.tpm.avgs.df %>% filter(mean >= 1) %>% .$gene_id

expressedGenes.ipsc = ipsc.rpkm.avgs.df %>% filter(mean >= 1) %>% .$gene_id
expressedGenes.mg = mg.rpkm.avgs.df %>% filter(mean >= 1) %>% .$gene_id
```
...
We compute FPKM values, and look at the distribution of these across all genes. We set a threshold of FPKM > 1 to identify "expressed genes".

```{r, warning=FALSE, message=FALSE, echo=FALSE}
ggplot(ipsc.rpkm.avgs.df, aes(x=mean, fill=(mean > 1))) + geom_histogram(bins=100) +
  theme_bw() + xlab("Mean FPKM") + scale_x_log10()
```

Next, we load the normalised data provided by Marc Bonder. These data were normalised by centering and scaling the values per gene across HIPSCI samples (mean 0, sd 1), and then 50 PCs were regressed out. The values were then rescaled to mean 0 and sd 1.

```{r LoadCountsNorm, warning=FALSE, message=FALSE, echo=FALSE}
expr.norm.df = readCounts(expr.norm.fname.mjb)
colnames(expr.norm.df)[1] = "gene_id"

# Ensure the columns are in the same order as the metadata rows
expr.norm.df = expr.norm.df %>% select(gene_id, meta.df$sample_name)

expr.norm.mat = expr.norm.df %>% select(-gene_id) %>% as.matrix()
rownames(expr.norm.mat) = expr.norm.df$gene_id

ipsc.expr.avgs.df = getExprSummary(expr.norm.mat[, ipsc_samples])
mg.expr.avgs.df = getExprSummary(expr.norm.mat[, mg_samples])

expr.norm.plot.df = expr.norm.df %>% gather(key="sample", value="val", -gene_id)
expr.norm.plot.df$expr = "low"
expr.norm.plot.df[expr.norm.plot.df$gene_id %in% (ipsc.rpkm.avgs.df %>% filter(mean > 1 & mean < 10) %>% .$gene_id), ]$expr = "med"
expr.norm.plot.df[expr.norm.plot.df$gene_id %in% (ipsc.rpkm.avgs.df %>% filter(mean >= 10) %>% .$gene_id), ]$expr = "high"
ggplot(expr.norm.plot.df, aes(x=val, fill=expr)) + geom_density(alpha=0.5) + theme_bw() +
  ggtitle("Normalised expression values (individual samples)")

ipsc.expr.avgs.plot.df = ipsc.expr.avgs.df
ipsc.expr.avgs.plot.df$expr = "low"
ipsc.expr.avgs.plot.df[ipsc.expr.avgs.plot.df$gene_id %in% (ipsc.rpkm.avgs.df %>% filter(mean > 1 & mean < 10) %>% .$gene_id), ]$expr = "med"
ipsc.expr.avgs.plot.df[ipsc.expr.avgs.plot.df$gene_id %in% (ipsc.rpkm.avgs.df %>% filter(mean >= 10) %>% .$gene_id), ]$expr = "high"
ggplot(ipsc.expr.avgs.plot.df, aes(x=mean, fill=expr)) + geom_density(alpha=0.5) + theme_bw() +
  ggtitle("Normalised expression values (averaged across samples)")
```

What we see is that for individual samples, there is quite a wide spread of normalised expression values. This is driven mainly by lowly-expressed genes where individual samples have outlier expression (e.g. Z-score +/-25). The high variance caused by these genes/samples leads the Z-scaling to more aggressively restrict values for most samples near to zero. When we take the average normalised expression across samples, the effect is less (most average values are between +/- 2) but lowly-expressed genes still have values closer to zero.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
ipsc.avgs.df = ipsc.rpkm.avgs.df %>%
  rename(rpkm = mean) %>%
  left_join(ipsc.expr.avgs.df %>% rename(norm = mean), by="gene_id")
ggplot(ipsc.avgs.df, aes(x=rpkm, y=norm)) + geom_point(alpha = 0.1) +
  theme_bw() + scale_x_log10()

# Previously I looked at the correlation between my read counts and Marc's. They differ
# substantially, so he must have used a different transcript annotation dataset.
# counts.avg.df = data.frame(gene_id = counts.df$gene_id, mean_js = rowMeans(counts.df  %>% select(-gene_id, -length)))
# counts.mjb.avg.df = data.frame(gene_id = counts.mjb.df$gene_id, mean_mjb = rowMeans(counts.mjb.df  %>% select(-gene_id)))
# counts.avg.df = counts.avg.df %>% left_join(counts.mjb.avg.df, by="gene_id")
# ggplot(counts.avg.df, aes(x=mean_mjb, y = mean_js)) + geom_point(alpha=0.1) +
...
We can do the same using the normalised expression data.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
# Compute the correlation between samples
expr_cor <- cor(na.omit(expr.norm.mat))
...
pheatmap(expr_cor, cluster_cols = T, cluster_rows = T, treeheight_col = 30,
         ...
         show_colnames = F, fontsize = 9, main = "CCDC6 RNA-seq sample correlations")
```
The differences between microglia and iPSCs are less clear, presumably because strongly expressed genes in one cell type can't contribute more to the correlations than other genes.

Let's ignore microglia for now, since we looked at them before, and cluster iPSCs separately, first using our own quantifications (FPKM).
...
Let's do the same using the normalised data.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
pheatmap(expr_cor[ipsc_samplenames, ipsc_samplenames],
         cluster_cols = T, cluster_rows = T, treeheight_col = 30,
         annotation_col = meta.df[meta.df$cell_type == "ipsc",] %>% select(condition, batch),
         show_colnames = F, fontsize = 9, main = "iPSC RNA-seq sample correlations (all genes)")

expressedGenes.ipsc.norm = expressedGenes.ipsc[expressedGenes.ipsc %in% rownames(expr.norm.mat)]
norm.ipsc.expressed = expr.norm.mat[expressedGenes.ipsc.norm, ipsc_samplenames]
expr_cor.ipsc.expressed <- cor(norm.ipsc.expressed)

titleStr = sprintf("iPSC RNA-seq sample correlations (%d expressed genes)", length(expressedGenes.ipsc.norm))
pheatmap(expr_cor.ipsc.expressed,
         cluster_cols = T, cluster_rows = T, treeheight_col = 30,
         annotation_col = meta.df[meta.df$cell_type == "ipsc",] %>% select(condition, batch),
         show_colnames = F, fontsize = 9, main = titleStr)
```
Using all genes is not very informative, and samples cluster completely by batch; since non-expressed genes contribute as much to the correlations as other genes, we should really restrict to expressed genes. Considering expressed genes, the sample clustering is similar to with non-normalised data, but not exactly. Samples still cluster largely by batch. Clone HOM_HDR_H11 no longer clusters in a tight group with WT_KOLF2_1/2/3 and L9/B9, though these remaining samples still cluster together (but not as tightly). Batch 2 HOM_KO_D3/G6/G9 still cluster together. 


Let's look at PCs.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
dds.ipsc = DESeqDataSetFromMatrix(countData = rawcounts.mat[expressedGenes.ipsc, meta.ipsc.df$sample_name],
...
ipsc.expr.vsd = assay(vsd.ipsc)[expressedGenes.ipsc, ]
pca.result = pcaMethods::pca(t(ipsc.expr.vsd), nPcs = 5)
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
pca = prcomp(t(assay(vsd.ipsc)[expressedGenes.ipsc,]))
gs_pca = gosummaries(pca, components=1:4, custom_bg = expressedGenes.ipsc)
plot(gs_pca, components = 1, fontsize = 12)
plot(gs_pca, components = 2, fontsize = 12)
plot(gs_pca, components = 3, fontsize = 12)
plot(gs_pca, components = 4, fontsize = 12)
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
# Plot showing genes
gs_pca2 = gosummaries(pca, show_genes = T, n_genes = 50, components=1:4, custom_bg = expressedGenes.ipsc)
plot(gs_pca2, components = 1, fontsize = 12)
plot(gs_pca2, components = 2, fontsize = 12)
plot(gs_pca2, components = 3, fontsize = 12)
plot(gs_pca2, components = 4, fontsize = 12)
```

Let's looks at PCs for the normalised expression data.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
pca.result = pcaMethods::pca(t(norm.ipsc.expressed), nPcs = 5)
#plotPCA(vsd.ipsc, intgroup="condition") + ggtitle("iPSC: PCA of normalized counts")
pcs = data.frame(scores(pca.result))
pcs$sample_name = rownames(pcs)
pcs.meta = dplyr::left_join(pcs, meta.ipsc.df, by="sample_name")
...
It looks as if KO samples from Batch 2 (and possibly Batch 1 as well) separate on PC3. WT1 and WT2 are fairly large outliers on PC4. The EN_HDR samples don't separate on any PC.

Let's examine the PC results by looking at enriched GO terms across the PCs.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
pca = prcomp(t(norm.ipsc.expressed))
gs_pca = gosummaries(pca, components=1:4, custom_bg = expressedGenes.ipsc.norm)
plot(gs_pca, components = 1, fontsize = 12)
plot(gs_pca, components = 2, fontsize = 12)
plot(gs_pca, components = 3, fontsize = 12)
plot(gs_pca, components = 4, fontsize = 12)
...
Look at the genes defining these PCs.
...
We save some results from gProfiler, particularly for PC3.


```{r, warning=FALSE, message=FALSE, echo=FALSE}
#Let's see what the heatmap looks like if we regress out PCs 1 and 2.
loadings = pcaMethods::loadings(pca.result)
ipsc.expr.vsd
all(rownames(loadings) == rownames(ipsc.expr.vsd))
ipsc.expr.vsd.pcacorr = 
  ```
...
Now look at CCDC6 normalised expression.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
ccdc6_ensg = "ENSG00000108091"

ccdc6.expr.norm = expr.norm.mat[ccdc6_ensg, ]
ccdc6.df = data.frame(sample_name = colnames(expr.norm.mat), expr = ccdc6.expr.norm) %>%
  left_join(meta.df, by="sample_name") %>%
  arrange(expr)
ggplot(ccdc6.df %>% filter(cell_type == "ipsc"), aes(x=fct_reorder(sample_short, -expr), y=expr, fill=condition2)) +
  geom_bar(stat="identity") +
  theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("CCDC6 expression (iPSCs) - normalised") + ylab("CCDC6 expression (normalised)") + xlab("Sample name") +
  scale_fill_discrete(name = "Condition")
```

Unfortunately, what looked like a promising trend in the basic counts data, with EN_HDR samples having higher CCDC6 expression, is gone in the normalised data. Indeed, WT samples cover basically the whole range of expression values (apart from the KOs), and the HDR samples for both EN and SYN are intermediate.
...
genes_near_ccdc6 = grch38 %>% filter(symbol %in% c("CCDC6", "ANK3", "SLC16A9", "AL592430.1"))
normalized_counts.ipsc <- counts(dds.ipsc, normalized = T)
sel_norm_counts <- normalized_counts.ipsc[genes_near_ccdc6$ensgene, meta.ipsc.df$sample_name] %>%
  t() %>% as.data.frame()
...
I don't see any pattern to the other genes based on CCDC6 expression This isn't surprising since we're looking at gene KOs, and there's no reason to expect other cis-effects. Let's order by edit type to see if e.g. enhancer deletion or HDR lines have nearby genes affected.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
# Get the counts, with samples ordered by edit type
sel_counts <- normalized_counts[genes_near_ccdc6$ensgene, meta.ipsc.df %>% arrange(condition2) %>% .$sample_name] %>%
  t() %>% as.data.frame() %>% t()
rownames(meta.ipsc.df) = meta.ipsc.df$sample_name
rownames(sel_counts) = genes_near_ccdc6$symbol
sel_counts2 = sel_counts

colnames(sel_counts) = meta.ipsc.df[colnames(sel_counts),]$sample_short
...
sel_counts.df = sel_counts2 %>% as.data.frame() %>% rownames_to_column(var = "gene") %>%
...
#print(sel_counts.df)
test_counts.df = sel_counts.df %>% mutate(edit = as.character(edit))
...
Now we do it again with the normalised data.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
# Get the counts, with samples ordered by edit type
sel_norm_counts <- expr.norm.mat[genes_near_ccdc6$ensgene, meta.ipsc.df %>% arrange(condition2) %>% .$sample_name] %>%
  t() %>% as.data.frame() %>% t()
rownames(meta.ipsc.df) = meta.ipsc.df$sample_name
rownames(sel_norm_counts) = genes_near_ccdc6$symbol
...
sel_counts.df = sel_norm_counts2 %>% as.data.frame() %>% rownames_to_column(var = "gene") %>%
  ...
  left_join(meta.ipsc.df %>% select(sample=sample_name, edit=condition2, batch, pc1_outlier), by="sample")
sel_counts.df2 = bind_rows(sel_counts.df,
                           sel_counts.df %>% filter(gene == "CCDC6", edit != "KO") %>% mutate(gene = "CCDC6 (no KOs)"))
ggplot(sel_counts.df2, aes(x=edit, y=expr, fill=edit)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width=0.2, alpha=0.67, size=2, aes(shape=batch)) +
  theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(~gene, nrow = 2, scales="free") +
  ggtitle("Genes near CCDC6 (normalised counts)")
...
There's a slight indication that EN_DEL samples might have lower expression than WT in normalised data, though I don't think this could be significant from looking at it. But there's no difference between EN_HDR and WT.

Let's do it all again after excluding 4 outlier samples.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
test_counts.no_outliers.df = test_counts.df %>% filter(!pc1_outlier)

print("EN_HDR vs. WT: test for difference in log counts using glm. The estimate is in log units, so 0.227 corresponds to a fold change of e^0.227 = 1.25.")
res = by(test_counts.no_outliers.df, INDICES = test_counts.no_outliers.df$gene, FUN = function(df) { glmFitVsWT(df, "EN_HDR") })
bind_rows(lapply(res, tidy), .id = "gene") %>% filter(term != "(Intercept)")

print("KO vs. WT: test for difference in log counts using glm")
res = by(test_counts.no_outliers.df, INDICES = test_counts.no_outliers.df$gene, FUN = function(df) { glmFitVsWT(df, "KO") })
bind_rows(lapply(res, tidy), .id = "gene") %>% filter(term != "(Intercept)")

print("SYN_HDR vs. WT: test for difference in log counts using glm")
res = by(test_counts.no_outliers.df, INDICES = test_counts.no_outliers.df$gene, FUN = function(df) { glmFitVsWT(df, "SYN_HDR") })
bind_rows(lapply(res, tidy), .id = "gene") %>% filter(term != "(Intercept)")
```
...
Let's do it again, but now using a randomization method, whereby I compare the test statistic to those from 1000 permutations of expression across the relevant groups.
...
doRandomizationTest = function(testgene, compare_edit, nperm = 1000) {
  test.df = test_counts.df %>% filter(gene == testgene, edit %in% c("WT", compare_edit))
  #print("EN_HDR vs. WT: RANDOMIZATION test for difference in log counts using glm.")
  main_fit = glmFitVsWT(test.df, compare_edit)
  coefName = paste0("edit", compare_edit)
  main_stat = coefficients(main_fit)[coefName]
  rand_stats = c()
  for (i in 1:nperm) {
    fit = glmFitVsWT(test.df %>% ungroup() %>% mutate(count = sample(count)), compare_edit)
    rand_stats = c(rand_stats, coefficients(fit)[coefName])
  }
  p = min(sum(main_stat < rand_stats), sum(main_stat > rand_stats)) / length(rand_stats)
  p_text = sprintf("one-tailed p = %.3g", p)
  ggp = ggplot(data.frame(rand_stats=rand_stats), aes(x=rand_stats)) +
    geom_histogram(bins = 40) + theme_bw() +
    geom_vline(xintercept = main_stat, size = 1, col = "red", alpha = 0.8) +
    annotate(geom = "text", x = Inf, y = Inf, label = p_text, hjust = 1.1, vjust = 2) +
    ggtitle(sprintf("%s: %s vs WT", testgene, compare_edit)) +
    theme(plot.title = element_text(size=11), axis.title.x = element_blank())
  res = list(gene = testgene, comparison = compare_edit, nperm = nperm, p = p, main_stat = main_stat, rand_stats = rand_stats, ggp = ggp)
  res
}

print("EN_HDR vs. WT: RANDOMIZATION test for difference in log counts using glm.")
set.seed(37)
gene_list = c("CCDC6", "AL592430.1", "ANK3", "SLC16A9")

reslist = lapply(gene_list, function(gene) doRandomizationTest(gene, "EN_HDR", nperm = 1000))
plotlist = lapply(reslist, function(res) res$ggp)
cowplot::plot_grid(plotlist = plotlist, ncol=3)

res.df = data.frame(gene = sapply(reslist, function(res) res$gene),
                    comparison = sapply(reslist, function(res) paste0(res$comparison," vs. WT")),
                    estimate = sapply(reslist, function(res) res$main_stat),
                    fold_change = sapply(reslist, function(res) exp(res$main_stat)),
                    permutation_p = sapply(reslist, function(res) res$p))
print(res.df)

reslist = lapply(gene_list, function(gene) doRandomizationTest(gene, "KO", nperm = 1000))
plotlist = lapply(reslist, function(res) res$ggp)
cowplot::plot_grid(plotlist = plotlist, ncol=3)
...
reslist = lapply(gene_list, function(gene) doRandomizationTest(gene, "SYN_HDR", nperm = 1000))
plotlist = lapply(reslist, function(res) res$ggp)
cowplot::plot_grid(plotlist = plotlist, ncol=3)
...
## Differential expression: HOM KO vs WT (including PC1)

Let's try again, this time including PC1 as a covariate.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
dds.ipsc = DESeqDataSetFromMatrix(countData = rawcounts.mat[, meta.ipsc.df$sample_name],
                                  colData = pcs.meta,
                                  design = ~ batch + PC1 + condition)
...
ggplot(ipsc.hom_ko.res.df) + 
  ...
  theme(legend.position = "none", 
        plot.title = element_text(size = rel(1.5), hjust = 0.5), 
        axis.title = element_text(size = rel(1.25))) +
  ggtitle("iPSC diffexp (incl PC 1) HOM KO:WT volcano plot")

if (saveFiles) {
  ipsc.hom_ko.res.df %>% write.table(file=file.path(outputPath, "iPSC_Hom_KO_vs_WT.PC1.DE_genes.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
  ipsc.hom_ko.res.df %>% filter(padj < 0.05) %>% write.table(file=file.path(outputPath, "iPSC_Hom_KO_vs_WT.PC1.DE_genes.sig.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
}
```
...
sig_norm_counts <- normalized_counts[ipsc.hom_ko.res.sig.df$gene_id, meta.ipsc.df$sample_name] %>%
  t() %>% as.data.frame() %>% t()
colnames(sig_norm_counts) = meta.ipsc.df[colnames(sig_norm_counts),]$sample_short
...
sig_norm_counts <- normalized_counts[ipsc.hom_ko.res.sig.df$gene_id, meta.ipsc.df %>% arrange(condition2) %>% .$sample_name] %>%
  t() %>% as.data.frame() %>% t()
colnames(sig_norm_counts) = meta.ipsc.df[colnames(sig_norm_counts),]$sample_short
...
pheatmap(sig_norm_counts,
         ...
         annotation_col = annot.df,
         scale = "row",
         main = "iPSC diffexp genes HOM KO:WT (by edit type)")
...
genesUp = (ipsc.hom_ko.res.sig.df %>% arrange(padj) %>% filter(log2FoldChange > 0) %>%
             filter(between(row_number(), 1, 1000)) %>% .$gene_id)
genesDown = (ipsc.hom_ko.res.sig.df %>% arrange(padj) %>% filter(log2FoldChange < 0) %>%colnames(sig_norm_counts)
             filter(between(row_number(), 1, 1000)) %>% .$gene_id)
gl = list(DiffExpInKO = list(Up=genesUp, Down=genesDown)) #
#gs1 = gosummaries(gl)
#plot(gs1, fontsize = 12)
...
gp.down = gprofiler(query = genesDown, custom_bg = expressedGenes.ipsc, organism = "hsapiens")
if (saveFiles) {
  gp.up %>% write.table(file=file.path(outputPath, "iPSC_Hom_KO_vs_WT.PC1.upregulated.gprofiler.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
  gp.down %>% write.table(file=file.path(outputPath, "iPSC_Hom_KO_vs_WT.PC1.downregulated.gprofiler.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
}
```

## Differential expression: HOM KO vs WT (including PCs 1 + 2)

Let's try again, this time including PCs 1 & 2 as covariates.

```{r, warning=FALSE, message=FALSE, echo=FALSE}
dds.ipsc = DESeqDataSetFromMatrix(countData = rawcounts.mat[, meta.ipsc.df$sample_name],
                                  colData = pcs.meta,
                                  design = ~ batch + PC1 + PC2 + condition)
...
if (saveFiles) {
  ipsc.hom_ko.res.df %>% write.table(file=file.path(outputPath, "iPSC_Hom_KO_vs_WT.PCs1_2.DE_genes.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
  ipsc.hom_ko.res.df %>% filter(padj < 0.05) %>% write.table(file=file.path(outputPath, "iPSC_Hom_KO_vs_WT.PCs1_2.DE_genes.sig.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
}
```
...
rpkm.mat = getRPKMs(normalized_counts.ipsc)
rpkm.df = cbind(data.frame(gene_id = rownames(rpkm.mat)), rpkm.mat)
ipsc.rpkm.avgs.df = getExprSummary(rpkm.mat)
expressedGenes.ipsc = ipsc.rpkm.avgs.df %>% filter(mean >= 1) %>% .$gene_id
...
pca.result = pcaMethods::pca(t(assay(expressedGenes.ipsc)[varGenes, ]), nPcs = 5)
pcs = data.frame(scores(pca.result))
pcs$sample_name = rownames(pcs)
pcs.meta = dplyr::left_join(pcs, meta.ipsc.df, by="sample_name")
...
## Differential expression: EN rs1171832 Hom HDR vs WT (including PC1)

This comparison involves 3 HDR samples vs. 6 WT samples, using PC1 as a covariate.
...
if (saveFiles) {
  ipsc.en_hom_hdr.res.df %>% write.table(file=file.path(outputPath, "iPSC_en_hom_hdr_vs_WT.PC1.DE_genes.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
  ipsc.en_hom_hdr.res.df %>% filter(padj < 0.05) %>% write.table(file=file.path(outputPath, "iPSC_en_hom_hdr_vs_WT.PC1.DE_genes.sig.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
}
```
...
gp.down = gprofiler(query = genesDown, custom_bg = expressedGenes.ipsc, organism = "hsapiens")
if (saveFiles) {
  gp.up %>% write.table(file=file.path(outputPath, "iPSC_en_hom_hdr_vs_WT.PC1.upregulated.gprofiler.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
  gp.down %>% write.table(file=file.path(outputPath, "iPSC_en_hom_hdr_vs_WT.PC1.downregulated.gprofiler.tsv"), sep="\t", quote=F, row.names=F, col.names=T)
}
```
