...
df = meta.df %>% dplyr::left_join(regions.df, by=c("SNP_number" = "index")) %>%
  dplyr::rename(amplicon_size = end) %>%
  dplyr::mutate(read_len = 150, amplicon_sd = 20,
                expected_overlap = 2*read_len - amplicon_size) %>%
  dplyr::mutate(output_file = paste0(Sample, ".extendedFrags.fastq.gz"))
...
df = df %>% dplyr::select(Sample, Read_1_file, Read_2_file, read_len, amplicon_size, amplicon_sd, min_overlap, max_mismatch_dens, output_file)
