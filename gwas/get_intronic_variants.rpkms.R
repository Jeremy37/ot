...
df = readr::read_tsv(file.path(root, "gwas/AD/finemap/AD.finemap.vep.transcribed.pcausal_gt_0.locusnames.txt")) %>%
...
write.table(df.ann, file.path(root, "gwas/AD/finemap/AD.finemap.vep.transcribed.pcausal_gt_0.locusnames.rpkms.txt"), row.names=F, col.names=T, quote=F, sep="\t")
...
snpChrPos.df = readr::read_tsv(file.path(root, "gwas/PD/finemap/PD.credible_sets.set")) %>%
