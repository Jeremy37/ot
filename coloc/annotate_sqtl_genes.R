...
sqtl.introns$chr = sapply(sqtl.introns$ensemblID, function(x) {strsplit(x, split=":", fixed=T)[[1]][1]})
sqtl.introns$start = as.integer(sapply(sqtl.introns$ensemblID, function(x) {strsplit(x, split=":", fixed=T)[[1]][2]}))
sqtl.introns$end = as.integer(sapply(sqtl.introns$ensemblID, function(x) {strsplit(x, split=":", fixed=T)[[1]][3]}))
sqtl.introns$cluster = sapply(sqtl.introns$ensemblID, function(x) {strsplit(x, split=":", fixed=T)[[1]][4]})
...
sqtl.introns.new = sqtl.introns %>% dplyr::left_join(sqtl.overlaps %>% dplyr::select(clusterKey, geneid, geneSymbol), by=c("ensemblID" = "clusterKey"))
write.table(sqtl.introns.new, file=paste0(sqtl_introns_fname, ".ann.txt"),
