...
#opt = list(g = "AD.IGAP1", w = "2e5", overlapdist = "1e5", phenotype = "mono_gene_eQTL", d = "/Users/jeremys/work/opentargets/datasets/GWAS", o = "/Users/jeremys/work/opentargets/coloc/blueprint/",
#           q = "/Users/jeremys/work/opentargets/datasets/blueprint/", s = 194, p1 = 1e-3, p2 = 1e-4, p12 = 5e-5)
...
GRCh37_variants = importVariantInfo(file.path(root, "datasets/blueprint/mono_gene_eQTL.variant_info.txt.gz"))
...
importBlueprintQTLs = function(min_pvalue_path) {
  col_names = c("phenotype_id","chr","pos","snp_id","p.value","beta","Bonferroni.p.value","p_fdr","MAF","std.error_of_beta")
...
constructQtlList <- function(phenotype, qtl_base, sample_size_list){
  ...
  for(condition in conditions){
    ...
    min_pvalues[[condition]] = importBlueprintQTLs(min_pvalue_path) %>% dplyr::select(phenotype_id, snp_id, p_fdr)
...
blueprintTabixFetchQTLs = function(phenotype_ranges, tabix_file) {
  ...
  qtl_columns = c("phenotype_id", "snp_chr", "snp_pos", "snp_id", "p_nominal",
                      "beta", "Bonferroni.p.value", "FDR", "MAF", "std.error_of_beta")
  qtl_coltypes = "ccicdddddd"
