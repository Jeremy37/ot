...
p2 = ggplot(merged.df, aes(x=DS_DL, y=hdr.effect)) +
  geom_point(alpha = 0.7) +
  geom_errorbar(aes(ymin=hdr.effect_confint_lo, ymax=hdr.effect_confint_hi), width=0.02, alpha=0.5) +
  xlab("SpliceAI donor loss score") + ylab("GenIE effect size") +
...
pdf("ccdc6.genie_splicing.all.pdf", width=7, height=6)
p1
p2
p3
...
pdf("ccdc6.genie_splicing.seqlogo.pdf", width=2.5, height=2)
p.genie.importance1
dev.off()

pdf("ccdc6.genie_splicing.scatter.pdf", width=2.5, height=2.3)
p2
dev.off()
